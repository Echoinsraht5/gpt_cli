...
opening = Text.from_markup(
    f"which function do you want to use? [bold italic magenta](1.chat 2.translate 3.edit 4.summarize)[/bold italic magenta]\nplease enter the number:")
print(opening)
num = input()
totalTokens = 0
maxTokens = 4096
...
class GPT3:
    ...
    def dcall(prompt, num, totalTokens, maxTokens, textori):
        ...
        if num == '2':
            ...
            if msg == 'change lang':
                ...
                return GPT3.gpt_turbo_chat(prompt, msg, num, totalTokens, maxTokens)
        elif num == '3':
            msg = input(
            ...
            msg = input(f'sentence or paragraph you want to sum up > ')
            if msg == 'change lang':
                editlan = input(
                    f"which language do you want to sum up to? > ")
                changedPrompt = f"{textori} Refine and summarize it concisely in {editlan}"
                GPT3.dcall(changedPrompt, num, totalTokens, maxTokens, textori)
            else:
                return GPT3.gpt_turbo_chat(prompt, msg, num, totalTokens, maxTokens)
    ...
    def recall(num, recur, totalTokens, maxTokens):
        ...
        if num == '4':
            if recur:
                text = input(
                    f'sentence or paragraph you want to sum up > ')
                editlan = input(f'which language do you want to sum up to? > ')
                GPT3.logi(editlan, num, totalTokens, maxTokens)
                prompt = f"{text} Refine and summarize it concisely in {editlan}"
                GPT3.gpt_turbo_chat(prompt, text, num, totalTokens, maxTokens)
            elif not recur:
                print(Text.from_markup(
                    f"[bold italic cyan]you have chosen summarize function, show me your text![/bold italic cyan]"))
                text = input(
                    f'sentence or paragraph you want to sum up > ')
                editlan = input(f'which language do you want to sum up to? > ')
                GPT3.logi(editlan, num, totalTokens, maxTokens)
    ...
    def logi(msg, numm, totalTokens, maxTokens):
        if msg:
            ...
            if msg == 'reset' and (numm == '2' or numm == '3' or numm == '4'):
            ...
            if numm == '1':
                ...
                msg2 = input(
                    f"please enter something to summarize > ")
                msg = msg2
                if msg2:
                    return msg, numm, totalTokens, maxTokens
                while not msg2:
        ...
        while True:
            ...
            if num == '2':
                ...
                if msg == 'change lang':
                    ...
                    changedPrompt = f"{text} Refine and summarize it concisely in {editlan}"
                    GPT3.dcall(changedPrompt, num, totalTokens,
                               maxTokens, textori=text)
                else:
                    GPT3.logi(msg, num, totalTokens, maxTokens)
